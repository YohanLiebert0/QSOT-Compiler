...
color1 = 'tab:blue'
ax1.set_xlabel('Observer Velocity (beta = v/c)', fontsize=14, fontweight='bold')
ax1.set_ylabel('Quantum Correlation', color=color1, fontsize=13)
ax1.plot(df['velocity'], df['quantum_correlation'], 
         'o-', color=color1, linewidth=2.5, markersize=7,
         label='Quantum Coherence', markerfacecolor='white', markeredgewidth=1.5)
...
ax2.plot(df['velocity'], df['memory_backflow'], 
         'x--', color=color2, linewidth=2, markersize=8,
         label='Memory Backflow', markeredgewidth=2)
ax2.tick_params(axis='y', labelcolor=color2)
ax2.set_ylim(0, df['memory_backflow'].max() * 1.3)

# Title
plt.title('Relativistic Degradation of Quantum Resources\n(QSOT Compiler v1.2.3)', 
...
if not critical.empty:
    ...
    ax1.text(v_c + 0.08, 0.9, 'Causal\nHorizon', fontsize=10, color='dimgray',
