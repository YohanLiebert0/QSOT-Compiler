...
# [Imports]
try:
    from qsot.utils.math_utils import random_density_matrix, trace_distance
except ImportError:
    # Fallback for direct execution
    from ..utils.math_utils import random_density_matrix, trace_distance
...
class Trace:
    ...
    def emit(self, step: str, payload: Dict[str, Any]) -> None: 
        entry = {"ts": time.time(), "step": step, "prev_hash": self.prev, "payload": payload}
...
def check_axiom1_linearity(chans, trials=16, tol_abs=1e-8, seed=42):
    """
    Verify F(pρ + (1-p)σ) ≈ pF(ρ) + (1-p)F(σ) for all channels.
    """
    np.random.seed(seed)
    max_diff = 0.0
    passed = True
...
# ––––– ASDP Entrypoint –––––
def run(rho0: np.ndarray, channels: List[KrausChannel], times=None, outdir="artifacts", 
        build_kd=True, compute_markov_metrics=True, 
        seed=42, tol_abs=1e-8, observer_velocity=0.0):
    ...
    if observer_velocity > 0 and boost_damping_channel:
        ...
        for ch in channels:
            if "PhaseDamping" in ch.name or "Damping" in ch.name: # Generalize for OscillatingDamping too?
                ...
                active_channels.append(KrausChannel(name=f"{ch.name}_Boosted", kraus=[new_k0, new_k1]))
            else:
    ...
    ax2_res = check_axiom2_conditionability(rho_qsot, active_channels, tol_abs=tol_abs)
    ...
    mem_report = compute_memory_kernel(rho_qsot, active_channels) if compute_memory_kernel else {"error": "memory_kernel missing"}

    # 4. Entanglement Analysis [v1.2.0 New]
    ent_report = {}
    if compute_correlation_profile:
        ent_report = compute_correlation_profile(rho_qsot)
        (out_path / "entanglement_report.json").write_text(json.dumps(ent_report, indent=2))
    ...
    kd_data = {
        "entries": [], 
        "metrics": {"kd_negativity_proxy": 0.0} 
    }
    (out_path / "kd_quasiprob.json").write_text(json.dumps(kd_data, indent=2))
    
    trace.emit("complete", {"status": "success", "entanglement": ent_report.get("avg_value", 0)})
