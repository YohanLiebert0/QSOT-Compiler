...
if TORCH_AVAILABLE:
    class QuantumOptimizer(torch.nn.Module):
        ...
        def get_projector(self, theta, phi):
            ...
            if psi.dim() == 0: psi = psi.unsqueeze(0)
...
def run_kd_optimization(state_path: str, out_path: str, steps=200, lr=0.1, patience=20, min_delta=1e-6):
    """
    ...
    if not TORCH_AVAILABLE:
        ...
        Path(out_path).write_text(json.dumps({"optimized_value": 0.0, "error": "torch_missing"}, indent=2))
        return
    ...
    try:
        ...
        Path(out_path).write_text(json.dumps({"optimized_value": 0.0, "error": str(e)}, indent=2))
        return
    ...
    for i in range(steps):
        ...
        if i % 50 == 0:
            print(f"  Step {i:03d}: KD Value = {val.item():.6f} | Best = {best_loss:.6f}")
    ...
    res = {
        ...
        "angles": {
            "basis_a": {"theta": float(model.theta_a.item()), "phi": float(model.phi_a.item())},
            "basis_b": {"theta": float(model.theta_b.item()), "phi": float(model.phi_b.item())}
        },
...
if __name__ == "__main__":
    ...
    ap.add_argument("--state", required=True, help="Path to qsot_state.npz")
