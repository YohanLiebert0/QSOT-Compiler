...
def plot_kd_heatmap(kd_json_path: Path, out_path: Path):
    ...
    plt.title(f"Kirkwood-Dirac Quasi-probability")
...
def plot_gate_metrics(report_path: Path, out_path: Path):
    ...
    ax1_val = rep.get("axiom1_report", {}).get("max_deviation", rep.get("axiom1_diff", 1e-9))
    ax2_val = rep.get("axiom2_report", {}).get("max_trace_deviation", rep.get("axiom2_diff", 1e-12))
    ...
    bars = plt.bar(names, values, color=['#3498db', '#e74c3c'])
...
def plot_memory_kernel(report_path: Path, out_path: Path):
    ...
    plt.axvline(x=depth + 0.5, color='red', linestyle='--', label=f'Depth {depth}')
...
def plot_entanglement_evolution(report_path: Path, out_path: Path):
    ...
    plt.figure(figsize=(6, 4))
    # Plot line
    plt.plot(t, profile, marker='o', linestyle='-', color='#8e44ad', linewidth=2, label=metric)
    # Fill area
...
if __name__ == "__main__":
    ...
    parser.add_argument("--dir", type=Path, required=True, help="Artifacts directory")
    ...
    if mem_path.exists():
        try:
            plot_memory_kernel(mem_path, args.dir / "viz_memory_kernel.png")
    ...
    if ent_path.exists():
        try:
            plot_entanglement_evolution(ent_path, args.dir / "viz_entanglement.png")
