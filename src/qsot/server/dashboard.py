...
with st.sidebar:
    ...
    velocity = st.slider(
        ...
        0.5,
        0.01,
        help="Simulates time dilation.",
    )
    fixture = st.selectbox(
        "Quantum Channel Model",
        [
            "depolarizing_then_phase_damping",
            "quantum_chaos",
            "correlated_noise_with_ancilla_memory",
        ],
...
if st.session_state.get("run_triggered", False):
    ...
    with st.spinner(f"Compiling Quantum States (v={velocity}c)..."):
        try:
            ...
            env = os.environ.copy()
            env["PYTHONPATH"] = (
                str(root_dir / "src") + os.pathsep + env.get("PYTHONPATH", "")
            )
            ...
            subprocess.run(
                [
                    sys.executable,
                    str(root_dir / "src/qsot/utils/visualizer.py"),
                    "--dir",
                    str(ARTIFACTS_DIR),
                ],
                check=True,
            ...
            subprocess.run(
                [
                    sys.executable,
                    str(root_dir / "src/qsot/core/optimizer.py"),
                    "--state",
                    str(ARTIFACTS_DIR / "qsot_state.npz"),
                    "--out",
                    str(ARTIFACTS_DIR / "optimization_result.json"),
                ],
                check=True,
                env=env,
            )
            ...
            subprocess.run(
                [
                    sys.executable,
                    str(root_dir / "src/qsot/core/protocol_gen.py"),
                    "--opt-result",
                    str(ARTIFACTS_DIR / "optimization_result.json"),
                    "--out",
                    str(ARTIFACTS_DIR / "LAB_PROTOCOL.txt"),
                ],
                check=True,
                env=env,
            )
...
if ARTIFACTS_DIR.exists():
    ...
    if gate_rep:
        kpi1.metric(
            "Axiom Integrity",
            "PASS" if gate_rep["pass"] else "FAIL",
        )
    if mem_rep:
        kpi2.metric(
            "Memory Depth",
            f"{mem_rep.get('depth', 0)} steps",
        )
    if ent_rep:
        # Shorten metric name for display
        raw_metric = ent_rep.get("metric", "Unknown")
        short_metric = raw_metric.split("(")[0].strip()

        kpi3.metric(
            "Avg Coherence",
            f"{ent_rep.get('avg_value', 0):.3f}",
        )
        kpi4.metric(
            "Metric Type",
            short_metric,
    ...
    with tab1:
        ...
        with c1:
            if (ARTIFACTS_DIR / "viz_kd_heatmap.png").exists():
                st.image(
                    str(ARTIFACTS_DIR / "viz_kd_heatmap.png"),
                    caption="Kirkwood-Dirac Q-Prob",
                )
        with c2:
            if (ARTIFACTS_DIR / "viz_gate_metrics.png").exists():
                st.image(
                    str(ARTIFACTS_DIR / "viz_gate_metrics.png"),
                    caption="Axiom Gate Metrics",
                )
    ...
    with tab2:
        ...
        with ec2:
            ...
            if (ARTIFACTS_DIR / "viz_entanglement.png").exists():
                st.image(
                    str(ARTIFACTS_DIR / "viz_entanglement.png"),
                    width="stretch",
                )
    ...
    with tab3:
        if (ARTIFACTS_DIR / "viz_memory_kernel.png").exists():
            st.image(
                str(ARTIFACTS_DIR / "viz_memory_kernel.png"),
                caption="Non-Markovian Memory Depth",
            )
